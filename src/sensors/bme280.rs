...
impl<I2C: embedded_hal_async::i2c::I2c> Sensor for Bme280<I2C> {
    async fn measure(&mut self, data: &mut SensorData) -> Result<(), SensorError> {
        match self.sensor.read_sample().await {
            Ok(sample) => {
                data.add_measurement("temperature", sample.temperature.unwrap());
                data.add_measurement("humidity", sample.humidity.unwrap());
                data.add_measurement("pressure", sample.pressure.unwrap());
                Ok(())
            }
